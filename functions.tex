...
$$\send^\sigma(msg_0^\sigma,\dots,msg_n^\sigma) \mapsto \emptysec$$
...
\TODO{Looks like the pub value has an integrity requirement.}

$$\dhpub(gen^{\{i\}}, psec^{\{c,i,f\}}) \mapsto pub^{\{i\}}$$

$$\dhsec(pub^{\{i\}}, psec^{\{c,i,f\}}) \mapsto ssec^{\{c,i\}} $$
